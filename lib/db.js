...
// Every review rates the lawyer on these five categories; the review's
// overall rating (and the lawyer's grade) is derived from them.
export const CATEGORIES = [
  { key: 'communication', label: 'Communication' },
  { key: 'responsiveness', label: 'Responsiveness' },
  { key: 'transparency', label: 'Case Transparency' },
  { key: 'fee_clarity', label: 'Fee Clarity' },
  { key: 'professionalism', label: 'Professionalism' },
];

// Review threads: client review -> one lawyer response -> one client rebuttal.
const reviewCols = db.prepare(`PRAGMA table_info(reviews)`).all().map((c) => c.name);
const newCols = [
  ['lawyer_response', 'TEXT'],
  ['lawyer_response_at', 'TEXT'],
  ['client_rebuttal', 'TEXT'],
  ['client_rebuttal_at', 'TEXT'],
  ...CATEGORIES.map((c) => [`rating_${c.key}`, 'INTEGER']),
];
for (const [col, type] of newCols) {
  if (!reviewCols.includes(col)) {
    try {
      ...
      // Another process (e.g. a parallel build worker) added it first.
      if (!/duplicate column/.test(e.message)) throw e;
    }
  }
}
// Reviews written before category ratings existed count their old overall
// rating for every category.
db.exec(
  CATEGORIES.map(
    (c) => `UPDATE reviews SET rating_${c.key} = rating WHERE rating_${c.key} IS NULL;`
  ).join('\n')
);
...
export function addReview({ lawyerId, userId, ratings, title, body }) {
  const existing = db
    .prepare(`SELECT * FROM reviews WHERE lawyer_id = ? AND user_id = ?`)
    .get(lawyerId, userId);
  // Once the lawyer has responded, the original review is locked — the
  // client's remaining move is the rebuttal.
  if (existing && existing.lawyer_response) {
    return { ok: false, reason: 'locked' };
  }
  const values = CATEGORIES.map((c) => ratings[c.key]);
  const overall = values.reduce((a, b) => a + b, 0) / values.length;
  const catCols = CATEGORIES.map((c) => `rating_${c.key}`);
  db.prepare(
    `INSERT INTO reviews (lawyer_id, user_id, rating, title, body, ${catCols.join(', ')})
     VALUES (?, ?, ?, ?, ?, ${catCols.map(() => '?').join(', ')})
     ...
     DO UPDATE SET rating = excluded.rating, title = excluded.title,
                   ...
                   ${catCols.map((c) => `${c} = excluded.${c}`).join(', ')}`
  ).run(lawyerId, userId, overall, title, body, ...values);
  return { ok: true };
}

// Average score per category across all of a lawyer's reviews.
export function categoryAverages(lawyerId) {
  const selects = CATEGORIES.map((c) => `AVG(rating_${c.key}) AS ${c.key}`).join(', ');
  const row = db
    .prepare(`SELECT ${selects} FROM reviews WHERE lawyer_id = ?`)
    .get(lawyerId);
  return CATEGORIES.map((c) => ({ ...c, avg: row[c.key] }));
}
...
export function getLawyerById(id) {
  return db.prepare(`${LAWYER_SELECT} WHERE l.id = ? GROUP BY l.id`).get(id);
}

// The lawyer gets exactly one response per review.
export function addLawyerResponse(reviewId, body) {
  const info = db
    .prepare(
      `UPDATE reviews
       SET lawyer_response = ?, lawyer_response_at = datetime('now')
       WHERE id = ? AND lawyer_response IS NULL`
    )
    .run(body, reviewId);
  return info.changes === 1;
}

// The client gets exactly one rebuttal, only after the lawyer has responded.
export function addClientRebuttal(reviewId, userId, body) {
  const info = db
    .prepare(
      `UPDATE reviews
       SET client_rebuttal = ?, client_rebuttal_at = datetime('now')
       WHERE id = ? AND user_id = ?
         AND lawyer_response IS NOT NULL AND client_rebuttal IS NULL`
    )
    .run(body, reviewId, userId);
  return info.changes === 1;
}
